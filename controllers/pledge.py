...
def edit():
    ...
    record = dict(description=pledge.pledge.description, value=pledge.pledge.value,rewards=pledge.pledge.rewardids(),projectid=pledge.pledge.projectid,id=13)
    ...
    if( form.process().accepted):
        for reward in pledge.pledge.rewardids():
            db((db.rewardpledge.pledgeid==int(pledge.pledge.idpledge)) & (db.rewardpledge.rewardid==int(reward))).select(db.rewardpledge.ALL).first().rawdelete()

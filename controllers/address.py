...
def edit():
    ...
    address = db((db.address.idaddress==addressid) & (db.address.userid == auth.user_id)).select(db.address.ALL).first()
...
@auth.requires_login
def index():
    addresses = db((db.address.userid == auth.user_id) & (db.country.code == db.address.countrycode)).select(db.address.ALL,db.country.ALL)
